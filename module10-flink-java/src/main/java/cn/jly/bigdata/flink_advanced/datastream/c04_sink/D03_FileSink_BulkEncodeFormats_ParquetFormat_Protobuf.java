...
import cn.jly.bigdata.flink_advanced.datastream.beans.Model;
 ...
 *
 * 本例子：
 *  示范将protobuf数据写入parquet文件格式
 * @author jilanyang
 * @date 2021/7/26 10:11
 * @package cn.jly.bigdata.flink_advanced.datastream.c04_sink
 * @class D03_FileSink_BulkEncodeFormats_ParquetFormat_Protobuf
 */
public class D03_FileSink_BulkEncodeFormats_ParquetFormat_Protobuf {
    public static void main(String[] args) throws Exception {
        ...
        // wordcount - WordCount是protobuf格式的
        SingleOutputStreamOperator<Model.WordCount> wordAndCountDs = StreamWordCountUtils.wordCount(fileDs, " ")
                .map(new MapFunction<Tuple2<String, Long>, Model.WordCount>() {
                    @Override
                    public Model.WordCount map(Tuple2<String, Long> value) throws Exception {
                        return Model.WordCount.newBuilder().setWord(value.f0).setCount(value.f1).build();
                    }
                });

        // 声明file sink 之parquetAvroSink
        // 类似地，可以像这样创建将 Protobuf 数据写入 Parquet 格式的 FileSink：
        FileSink<Model.WordCount> parquetProtobufSink = FileSink.forBulkFormat(
                new Path("e:/flink/file_sink/parquet_protobuf"),
                ParquetProtoWriters.forType(Model.WordCount.class)
        ).build();

        // 执行file sink 之parquetAvroSink
        wordAndCountDs.sinkTo(parquetProtobufSink);

        env.execute("D03_FileSink_BulkEncodeFormats_ParquetFormat_Protobuf");
    }
}
