...
public class D04_TableApi_Window {
	public static void main(String[] args) throws Exception {
		...
		SingleOutputStreamOperator<Order> orderDs = env.socketTextStream(host, port)
				.map(new MapFunction<String, Order>() {
					...
					public Order map(String value) throws Exception {
						...
						return new Order(fields[0], fields[1], Long.parseLong(fields[2]),
		...
		// 创建表
		Table resTable = tableEnv.fromDataStream(orderWithWatermarkDs, $("orderId"), $("userId"),
						$("createTime").rowtime().as("rowtime"), $("money"))
				.window(Tumble.over(lit(5).seconds()).on($("rowtime")).as("w")).groupBy($("w"), $("userId"))
				.select($("userId"), $("orderId").count().as("order_count"), $("money").max().as("max_money"),
