 ...
 * @Author jilanyang
 * @Package cn.spark.streaming.c03_transform
 * @Class D03_UpdateStateByKeyDemo
 * @Date 2021/6/8 0008 19:38
 */
object D03_UpdateStateByKeyDemo {

  def main(args: Array[String]): Unit = {
    System.setProperty("hadoop.home.dir", "D:\\software\\devsoft\\hadoopBin")

    val sparkConf: SparkConf = new SparkConf().setMaster("local[*]").setAppName("D03_UpdateStateByKeyDemo")
    ...
    val stateStream: DStream[(String, Int)] = wordAndOneStream.updateStateByKey[Int](updateFunc)
    stateStream.print()
  ...
  /**
   * 义更新状态方法，参数 values 为当前批次单词频度，state 为以往批次单词频度
   *
   * @return 返回状态更新函数
   */
  def updateFunc: (Seq[Int], Option[Int]) => Option[Int] = {
    // values是新来的一个批次的数据集合，state是状态
