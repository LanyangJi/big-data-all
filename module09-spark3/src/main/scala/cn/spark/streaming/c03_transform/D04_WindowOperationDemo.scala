 ...
 * @Author jilanyang
 * @Package cn.spark.streaming.c03_transform
 * @Class D04_WindowOperationDemo
 * @Date 2021/6/9 0009 10:55
 */
object D04_WindowOperationDemo {
  def main(args: Array[String]): Unit = {
    System.setProperty("hadoop.home.dir", "D:\\software\\devsoft\\hadoopBin")

    // 需求：每6秒统计一下过去12秒的单词个数
    val sparkConf: SparkConf = new SparkConf().setMaster("local[*]").setAppName("D04_WindowOperationDemo")
    ...
    val windowStream: DStream[(String, Int)] = streamingContext.socketTextStream("linux01", 9999)
      ...
      .reduceByKeyAndWindow(
        (a: Int, b: Int) => a + b,
        Seconds(12),
        Seconds(6) // 当滑动步长大于等于窗口大小的时候，就没有重复数据
      )
